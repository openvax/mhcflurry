...
- **`mhcflurry eval paper-figures external-predictors`** — Optional adapter
  for the previous hook. Its initial runner shells out to the local `mhctools`
  executable and joins the long mhctools output back into the canonical wide
  benchmark table. This avoids a package dependency cycle: `mhctools` depends
  on MHCflurry, so MHCflurry does not import or require it.
