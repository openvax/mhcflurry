...
"""Plot training loss curves for every network in a run + highlight
...
def _load_manifest_curves(manifest_path):
    ...
    for _, r in df.iterrows():
        ...
        for fit_rec in fit_info:
            ...
            model_row["work_item_name"] = tinfo.get(
                "work_item_name", model_row["work_item_name"])
...
def _selection_key(model):
    """Return the stable identity of a candidate across selection cloning."""
    if model["work_item_name"]:
        return ("work_item_name", model["work_item_name"])
    training_coordinates = (
        model["fold"],
        model["arch_num"],
        model["replicate"],
    )
    if all(value is not None for value in training_coordinates):
        return ("training_coordinates",) + training_coordinates
    return ("model_name", model["model_name"])


def _plot_all_curves(selected_keys, all_models, out_path, title_suffix=""):
    ...
    for m in all_models:
        is_selected = _selection_key(m) in selected_keys
        label = None
        if is_selected:
...
def _plot_per_fold(selected_keys, all_models, out_path):
    ...
    for ax, fold in zip(axes, folds):
        ...
        for m in fold_models:
            ...
            if not val_curve:
                continue
            if _selection_key(m) in selected_keys:
                color = "#d62728"
                alpha = 0.95
                lw = 1.4
            else:
...
def make_parser(prog="mhcflurry train plot-loss-curves"):
...
def run(args):
    ...
    selected_keys = {_selection_key(m) for m in selected}
...
def run_argv(argv=None, prog="mhcflurry train plot-loss-curves"):
    """Parse ``argv`` and generate training-loss diagnostics."""
    return run(make_parser(prog).parse_args(argv))


def main():
    """Compatibility entry point for the repository script."""
    return run_argv()
